...
ssh -n $USER@$TARGET_EXT "cd wallaroo ; ./testing/performance/apps/market-spread/market-spread -i ${SERVER1}:7000,${SERVER1}:7001 -o ${SERVER1}:5555 -m ${SERVER1}:5001 -c ${SERVER1}:12500 -n worker${SOURCE_WORKER} --my-control ${TARGET}:3131 --my-data ${TARGET}:3132 --ponynoblock --resilience-disable-io-journal > /tmp/run-dir/market-spread${SOURCE_WORKER}.`date +%s`.out 2>&1" > /dev/null 2>&1 &
