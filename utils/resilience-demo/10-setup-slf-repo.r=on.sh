...
if [ "$SKIP_MAKE" = "" ]; then
    ssh -n $USER@$SERVER1_EXT "cd wallaroo ; make PONYCFLAGS='--verbose=1 -d' resilience=on build-testing-performance-apps-market-spread build-giles-all build-utils-cluster_shutdown"
fi
