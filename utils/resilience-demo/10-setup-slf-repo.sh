...
if [ "$SKIP_MAKE" = "" ]; then
    ssh -n $USER@$SERVER1_EXT "cd wallaroo ; make clean ; git checkout $REPO_BRANCH ; make PONYCFLAGS='--verbose=1 -d' resilience=off build-testing-performance-apps-market-spread build-giles-all build-utils-cluster_shutdown"
fi
