...
ssh -n $USER@$SERVER1_EXT "cd wallaroo ; ./testing/performance/apps/market-spread/market-spread -i ${SERVER1}:7000,${SERVER1}:7001 -o ${SERVER1}:5555 -m ${SERVER1}:5001 -c ${SERVER1}:12500 -d ${SERVER1}:12501 -t -e ${SERVER1}:5050 -w 2 $W_DOS_SERVER_ARG  --ponynoblock  > /tmp/run-dir/market-spread1.out 2>&1" &
sleep 2
